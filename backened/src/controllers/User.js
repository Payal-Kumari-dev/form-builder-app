...
const authenticateUser = async (req, res, next) => {
    try {
        ...
        if (!existingUser) {
            throw Object.assign(Error("Account not found with provided email"), { code: 404 });
        }

        const isPasswordValid = await bcrypt.compare(password, existingUser.password);
        if (!isPasswordValid) {
            throw Object.assign(Error("Invalid credentials provided"), { code: 401 });
        }

        const accessToken = jwt.sign({ uid: existingUser._id }, process.env.JWT_SECRET);
        res.status(200).json({ status: "success", msg: "Authentication successful", accessToken });
    } catch (err) {
        next(err);
    }
};
