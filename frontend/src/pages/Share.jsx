...
function Share() {
    ...
    return (
        <section className={styles.share_Layout} style={{ background: formData.formTheme }}>
            {shareBox.length > 0 && (
                <div className={`${styles.chat_box} ${cstyles.chat_box}`}>
                    {shareBox.map((item, index) => (
                        <div key={item.key + index} className={cstyles[item.data.role]}>
                            {item.data.role === 'admin' ? (
                                <>
                                    <img className={cstyles.chat_Head} src="/images/vectors/chat-head.png" alt="admin chat-head" />
